import Card from "../components/Card"
import { useRef } from "react";
import { Globe } from "../components/globe";
import CopyEmailButton from "../components/CopyEmailButton";
import { Frameworks } from "../components/Frameworks";

const About = () => {
    const grid2Container = useRef();
    const devOpsPrinciples = [
    "Collaboration",
    "Continuous Integration",
    "Continuous Delivery",
    "Automation",
    "Infrastructure as Code",
    "Monitoring & Feedback",
    "Continuous Testing",
    "Agile & Lean",
    "DevSecOps",
    "Continuous Improvement",
    ];

    const randomStyle = () => ({
    rotate: `${Math.floor(Math.random() * 60 - 30)}deg`, // -30° to +30°
    top: `${Math.floor(Math.random() * 80)}%`, // 0–80% vertical
    left: `${Math.floor(Math.random() * 80)}%`, // 0–80% horizontal
    position: "absolute",
    transition: "transform 0.4s ease",
  });

  return (
    <section className="c-space section-spacing">
        <h2 className="text-heading">About Me</h2>

        <div className="grid grid-cols-1 gap-4 md:grid-cols-6 md:auto-rows-[18rem] mt-12">
            {/* Grid 1 */}
            <div className="flex items-end grid-default-color grid-1">
                <img src="assets/coding-pov.png" className="absolute scale-[1.75] -right-[5rem] -top-[1rem] md:scale-[3] md:left-50 md:inset-y-10 lg:scale-[2.5]" />
                <div className="z-10">
                    <p className="headtext">Hi, I'm Walid Assknid</p>
                    <p className="subtext">I'm a computer science student passionate about infrastructure and cloud technologies. I enjoy exploring how systems work behind the scenes — from servers and networks to scalable cloud architectures. My goal is to build reliable, efficient, and secure infrastructures that power the next generation of digital innovation.</p>
                </div>
                <div className="absolute inset-x-0 pointer-events-none -bottom-4 h-1/2 sm:h-1/3 bg-gradient-to-t from-indigo" />
            </div>
            {/* Grid 2 */}
            <div className="grid-default-color grid-2">
                <div ref={grid2Container} className="flex items-center justify-center w-full h-full">
                    <p className="flex items-end text-5xl text-gray-500">CODE IS CRAFT</p>
                    {devOpsPrinciples.map((principle, i) => (
                        <Card key={i} style={randomStyle()} text={principle} containerRef={grid2Container} />
                    ))}
                    {/*<Card style={{rotate: "30deg", top: "70%", left: "70%" }} image="assets/logos/aws.svg" containerRef={grid2Container}/>
                    <Card style={{rotate: "-45deg", top: "70%", left: "25%" }} image="assets/logos/Ansible.svg" containerRef={grid2Container}/>
                    <Card style={{rotate: "-45deg", top: "5%", left: "10%" }} image="assets/logos/Go.svg" containerRef={grid2Container}/>
                    <Card style={{rotate: "0deg", top: "20%", left: "75%" }} image="assets/logos/Devops-toolchain.svg" containerRef={grid2Container}/>
                    <Card style={{rotate: "45deg", top: "40%", left: "80%" }} image="assets/logos/docker.svg" containerRef={grid2Container}/>
                    <Card style={{rotate: "15deg", top: "10%", left: "50%" }} image="assets/logos/kuber.svg" containerRef={grid2Container}/>*/}
                </div>
            </div>
            {/* Grid 3 */}
            <div className="grid-black-color grid-3">
                <div className="z-10 w-[50%]">
                    <p className="headtext">Time Zone</p>
                    <p>I'm based in Toulouse (France), and open to remote work/internship worldwide</p>
                </div>
                <figure className="absolute left-[30%] top-[10%]">
                    <Globe />
                </figure>
            </div>

            {/* Grid 4 */}
            <div className="grid-special-color grid-4">
                <div className="flex flex-col items-center justify-center gap-4 size-full">
                    <p className="text-center headtext">Do you want to start a project together?</p>
                    <CopyEmailButton />
                </div>
            </div>
            {/* Grid 5 */}
            <div className="grid-default-color grid-5">
                <div className="z-10 w-[50%]">
                    <p className="headText">Tech Stack</p>
                    <p className="subtext">I specialize in a variety of languages, frameworks, and tools that
              allow me to build robust and scalable applications</p>
                </div>
                <div className="absolute inset-y-0 md:inset-y-9 w-full h-full start-[50%] md:scale-125">
                    <Frameworks />
                </div>
            </div>
        </div>
    </section>
  )
}

export default About